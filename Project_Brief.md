...
**Version:** 1.0 · **Target DB:** PotgreeSQL · **Modules:** Access Control & Menu, User Management, Product Catalog, Supplier, Audit Logs
...
- **Database:** PostgreeSql 8.0 (InnoDB, utf8mb4_0900_ai_ci), Flyway/Liquibase untuk migration.
...
- **Packaging:** Docker Compose (api + pgsql + web).

### High-Level Diagram
```
React (Bootstrap) → REST API /api/v1 → Spring Boot Service → JPA → PostgreeSql
...
Referensi struktur kolom & tipe: lihat _database_pgsql.md_.
...
### 7.3 Migrations
- **Flyway**: `V1__baseline.sql` (DDL dari `database_pgsql.md`), `V2__seed.sql` (seed minimal).
...
- **Docker**: `docker-compose.yml` (pgsql, api, web).
- **CI/CD**: Lint/test → build Docker → push registry → deploy.
- **Config**: Secrets (DB pass, JWT secret) via env vars.
- **Backups**: pgsqldump harian untuk prod, rotasi 7/30 hari.

---

## 12) Testing Strategy
- **Backend**: Unit (JUnit5/Mockito), Integration (Testcontainers PostgreeSQL), Contract tests (OpenAPI).
